import React from "react";
import DragDrop from "./Components/DragDrop/DragDrop";
import {
  Box,
  FormControl,
  FormControlLabel,
  Grid,
  Paper,
  Radio,
  RadioGroup,
  Table,
  TableBody,
  TableCell,
  TableContainer,
  TableHead,
  TableRow,
} from "@mui/material";
import { useTranslation } from "react-i18next";
import { useTheme } from "@emotion/react";

function createData(name, date, size) {
  return { name, date, size };
}

const rows = [
  createData("File1", "Jan 4, 2024", "200 KB"),
  createData("File2", "Jan 4, 2024", "720 KB"),
  createData("File3", "Jan 4, 2024", "16 MB"),
];

export default function Services() {
  const theme = useTheme();
  const { t } = useTranslation();
  const { title, Desc, ratio1, ratio2, ratio3, ratio4, choose } =
    t("MainFiles/Profile");

  return (
    <>
      <Grid
        sx={{
          textAlign: theme.direction == "ltr" ? "right" : "left",
          paddingBottom: "2rem",
        }}
      >
        <Box
          sx={{
            fontFamily: "Tajawal",
            fontSize: "1.9rem",
            fontWeight: "700",
            lineHeight: "38px",
            color: "#1A1A1A",
          }}
        >
          {title}
        </Box>
        <Box
          sx={{
            fontFamily: "Tajawal",
            fontSize: "1rem",
            fontWeight: "400",
            lineHeight: "24px",
            color: "#4F4F4F",
          }}
        >
          {Desc}
        </Box>
      </Grid>
      <GroupRatio ooptions={{ ratio1, ratio2, ratio3, ratio4, choose }} />
      <DragDrop />
      <FilesTable />
    </>
  );
}

export function GroupRatio({
  ooptions: { ratio1, ratio2, ratio3, ratio4, choose },
}) {
  return (
    <>
      <FormControl
        sx={{
          width: "100%",
          display: "flex",
          justifyContent: "center",
          alignItems: "center",
        }}
      >
        <RadioGroup
          row
          aria-labelledby="demo-row-radio-buttons-group-label"
          name="row-radio-buttons-group"
          defaultValue="TRN Number"
          sx={{
            "& .MuiTypography-root": {
              fontFamily: "Tajawal",
              fontWeight: 400,
              fontSize: "16px",
            },
          }}
        >
          <FormControlLabel value={ratio1} control={<Radio />} label={ratio1} />
          <FormControlLabel value={ratio2} control={<Radio />} label={ratio2} />
          <FormControlLabel value={ratio3} control={<Radio />} label={ratio3} />
          <FormControlLabel value={ratio4} control={<Radio />} label={ratio4} />
        </RadioGroup>
        <Box
          sx={{
            color: "#4F4F4F",
            fontFamily: "Tajawal",
            fontWeight: 400,
            fontSize: "16px",
            marginBottom: "2rem",
          }}
        >
          {choose}
        </Box>
      </FormControl>
    </>
  );
}

function FilesTable() {
  const theme = useTheme();
  const { t } = useTranslation();
  const { MainTitle, MainDesc, TableTitle1, TableTitle2, TableTitle3 } =
    t("FilesTable/Profile");
  return (
    <>
      <Grid
        sx={{
          textAlign: theme.direction == "ltr" ? "right" : "left",
          border: "1px solid #E0E0E0",
          borderRadius: "10px",
          overflow: "hidden",
        }}
      >
        <Box
          sx={{
            color: "#1A1A1A",
            fontFamily: "Tajawal",
            fontWeight: 700,
            fontSize: "18px",
            padding: "1rem  1rem 0rem 1rem",
          }}
        >
          {MainTitle}
        </Box>
        <Box
          sx={{
            color: "#4F4F4F",
            fontFamily: "Tajawal",
            fontWeight: 400,
            fontSize: "14px",
            marginBottom: "1rem",
            padding: "1rem  1rem 0rem 1rem",
          }}
        >
          {MainDesc}
        </Box>
        <TableContainer component={Paper}>
          <Table sx={{ minWidth: 700 }} aria-label="caption table">
            <TableHead sx={{ background: "#E0E0E0" }}>
              <TableRow>
                <TableCell
                  sx={{
                    width: "20%",
                    fontFamily: "Tajawal",
                    fontWeight: 500,
                    fontSize: "16px",
                    color: "#4F4F4F",
                  }}
                  align="center"
                >
                  {TableTitle3}
                </TableCell>

                <TableCell
                  sx={{
                    width: "20%",
                    fontFamily: "Tajawal",
                    fontWeight: 500,
                    fontSize: "16px",
                    color: "#4F4F4F",
                  }}
                  align="center"
                >
                  {TableTitle2}
                </TableCell>
                <TableCell
                  sx={{
                    width: "50%",
                    fontFamily: "Tajawal",
                    fontWeight: 500,
                    fontSize: "16px",
                    color: "#4F4F4F",
                  }}
                  align={theme.direction == "ltr" ? "right" : "left"}
                >
                  {TableTitle1}
                </TableCell>
              </TableRow>
            </TableHead>
            <TableBody sx={{ maxHeight: "40px", overflowY: "hidden" }}>
              {rows.map((row) => (
                <TableRow key={row.name} sx={{}}>
                  <TableCell
                    align="center"
                    sx={{
                      fontFamily: "Tajawal",
                      fontWeight: 400,
                      fontSize: "14px",
                      color: "#4F4F4F",
                    }}
                  >
                    {row.size}
                  </TableCell>
                  <TableCell
                    align="center"
                    sx={{
                      fontFamily: "Tajawal",
                      fontWeight: 400,
                      fontSize: "14px",
                      color: "#4F4F4F",
                    }}
                  >
                    {row.date}
                  </TableCell>
                  <TableCell
                    component="th"
                    scope="row"
                    align="center"
                    sx={{
                      fontFamily: "Tajawal",
                      fontWeight: 400,
                      fontSize: "14px",
                      color: "#4F4F4F",
                      display: "flex",
                      justifyContent: "end",
                      alignItems: "center",
                    }}
                  >
                    {row.name}
                    <Box m={1}>
                      <svg
                        xmlns="http://www.w3.org/2000/svg"
                        width="40"
                        height="40"
                        viewBox="0 0 40 40"
                        fill="none"
                      >
                        <path
                          d="M7.75 4C7.75 2.20508 9.20508 0.75 11 0.75H27C27.1212 0.75 27.2375 0.798159 27.3232 0.883885L38.1161 11.6768C38.2018 11.7625 38.25 11.8788 38.25 12V36C38.25 37.7949 36.7949 39.25 35 39.25H11C9.20507 39.25 7.75 37.7949 7.75 36V4Z"
                          fill="white"
                          stroke="#E0E0E0"
                          stroke-width="1.5"
                        />
                        <path
                          d="M27 0.5V8C27 10.2091 28.7909 12 31 12H38.5"
                          stroke="#E0E0E0"
                          stroke-width="1.5"
                        />
                        <rect
                          x="1"
                          y="18"
                          width="26"
                          height="16"
                          rx="2"
                          fill="#D92D20"
                        />
                        <path
                          d="M4.8323 30V22.7273H7.70162C8.25323 22.7273 8.72316 22.8326 9.11142 23.0433C9.49967 23.2517 9.7956 23.5417 9.9992 23.9134C10.2052 24.2827 10.3082 24.7088 10.3082 25.1918C10.3082 25.6747 10.204 26.1009 9.99565 26.4702C9.78732 26.8395 9.48547 27.1271 9.09011 27.3331C8.69712 27.5391 8.22127 27.642 7.66255 27.642H5.83372V26.4098H7.41397C7.7099 26.4098 7.95375 26.3589 8.14551 26.2571C8.33964 26.1529 8.48405 26.0097 8.57875 25.8274C8.67581 25.6428 8.72434 25.4309 8.72434 25.1918C8.72434 24.9503 8.67581 24.7396 8.57875 24.5597C8.48405 24.3774 8.33964 24.2365 8.14551 24.1371C7.95138 24.0353 7.70517 23.9844 7.40687 23.9844H6.36994V30H4.8323ZM13.885 30H11.3069V22.7273H13.9063C14.6379 22.7273 15.2676 22.8729 15.7955 23.1641C16.3235 23.4529 16.7295 23.8684 17.0136 24.4105C17.3 24.9527 17.4433 25.6013 17.4433 26.3565C17.4433 27.1141 17.3 27.7652 17.0136 28.3097C16.7295 28.8542 16.3211 29.272 15.7884 29.5632C15.2581 29.8544 14.6237 30 13.885 30ZM12.8445 28.6825H13.8211C14.2757 28.6825 14.658 28.602 14.9681 28.4411C15.2806 28.2777 15.515 28.0256 15.6713 27.6847C15.8299 27.3414 15.9092 26.8987 15.9092 26.3565C15.9092 25.8191 15.8299 25.38 15.6713 25.0391C15.515 24.6982 15.2818 24.4472 14.9717 24.2862C14.6615 24.1252 14.2792 24.0447 13.8247 24.0447H12.8445V28.6825ZM18.5823 30V22.7273H23.3976V23.995H20.1199V25.728H23.078V26.9957H20.1199V30H18.5823Z"
                          fill="white"
                        />
                      </svg>
                    </Box>
                  </TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
        </TableContainer>
      </Grid>
    </>
  );
}
